...
const Song = ({ onClick, image, title, artist, album, year, previewAudio }) => {
    ...
    <Flex p="10px"align="center">
        ...
        <Spacer />
        <Box  m={['5px', '5px']}width="100px" textAlign="center">
            <Text fontWeight="bold" fontSize={['11px', '14px']}>{album}</Text>
        </Box>
        <Spacer />
        <Box  d={['none', 'block']}m={['5px', '5px']}width="100px" textAlign="center">
            <Text fontWeight="bold" fontSize={['11px', '14px']}>{year}</Text>
        </Box>
        <Spacer />
